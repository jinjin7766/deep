...
# 사물 검출 버튼 클릭 이벤트 처리
if st.button("사물 검출 실행") and uploaded_file and model_file: # 버튼을 눌렀고 영상도 업로드 했고 모델도 업로드 했으면 
    with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as temp_output: 
        output_path = temp_output.name  # 임시 비디오 파일을 생성하고 output path 에 저장해라  
    ...
    while cap.isOpened():  # 비디오가 끝날때 까지 프레임을 하나씩 읽어옵니다. 
        ...
        # 검춝된 객체가 있으면 detections 에 그 정보가 들어가고 없으면 빈 리스트를 반환
        ...
        if len(detections) > 0:     # 만약 detections 에 값이 있다면 
            ...
            # 검출 결과가 없을 때 로그 출력
            st.write(f"Frame {frame_count}: No detections") # 검출이 안되었으면 no detecction 메세지 출력 

        out.write(frame)  # out 비디오 파일에 기록을하고 
